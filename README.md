...
On Windows, use a console attached to stdin and stdout (Windows Terminal is recommended). Redirected streams are rejected. The backend owns duplicate console handles and permits one `Terminal` per process, including while suspended. Native key records preserve modifiers and distinguish Ctrl+I from Tab; `InputMode` affects the byte parser, not native console records. Paste arrives as key records. See the [Windows validation report](docs/WINDOWS_VALIDATION.md) for tested hosts, build commands, and native input details.
